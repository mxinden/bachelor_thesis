...
\begin{content}
  ...
  \todo[inline]{Filter über Kennzahlen in zwei Gruppen aufteilen: Filter mit
  Ober- und Untergrenze und Filter mit bestehen bzw nicht bestehen. Auch in
  \textit{DataFurnace}-Sprache entsprechend ändern}
  ...
  \paragraph{Tabellenkomponente}\label{paragraph:tabellenkomponente} Die Fragestellungen eines Nutzers der
  ...
  der Software anhand dieser drei Ansichten respektive Schritte erklärt. Der
  Wechsel zwischen den Ansichten gelingt über die Kopfleiste der
  \textit{DataFurnace}-Software, in welcher zu jeder Ansicht ein Link
  abgebildet ist.
  ...
  oberen Teil der Box, welche für jedes neues Geschäftsobjekt erscheint, kann
  der Name eines Geschäftsobjekts gesetzt und das gesamte Objekt gelöscht
  werden. Für die weitere Erläuterung der Ansicht wird das Wort Element als
  Generalisierung von Dimensionen, Dimensionsleveln und Attributen verwendet.
  In der Box können des Weiteren Dimensionen dem Geschäftsobjekt hinzugefügt
  werden, welchen wiederum beliebig viele Dimensionslevel und schlussendlich
  ein Attribut untergeordnet werden kann. Einem Geschäftsobjekt können folglich
  beliebig viele Dimensionen angehören. Eine Dimension kann in beliebig viele
  Dimensionslevel aufgeteilt werden. Einem Dimensionslevel können weitere
  Dimensionslevel oder ein Attribut folgen. Eine Dimension-Dimensionslevel
  Kette muss mit einem Attribut enden. Einem Attribut können keine weiteren
	Elemente folgen. Ein Beispiel ist in Abbildung~\ref{fig:baumstruktur-produkt}
	zu sehen.
  ...
  werden und die gesamte Ausprägung kann durch einen Klick auf das
  \textit{Löschen}-Symbol gelöscht werden.
  ...
  Berichtsmodellierung. Sie unterteilt sich horizontal in den tabellarischen
  Bericht (Tabellenkomponente) selbst und den spezifizierten Filtern
  (Filterkomponente) darunter. Zunächst soll auf die Filterspezifikation
  eingegangenn werden. Wie in Abschnitt~\ref{paragraph:filternde-komponente}
  beschrieben kann ein Filter sowohl auf ein Dimensionslevel, als auch eine
  Kennzahl, basieren. 

  \paragraph{Filterkomponente}
  
  Will ein Nutzer eine neue Kennzahl auf Basis eines
  ...
  Kennzahl über der Filterkomponente los gelassen wird, wird ein Filter auf Basis der
  Kennzahl erstellt. Nun kann der Nutzer im Filter spezifizieren, bei welchen
  Ergebnissen der Kennzahl er ausfiltern möchte, und bei welchen Ergebnissen
  eine Geschäftsobjektinstanz weiter kommt, indem er \zB{} Ober- und
  Untergrenzen definiert, oder entscheidet ob Geschäftsobjektinstanzen die
  Prüfung der Kennzahl bestehen müssen, oder nicht bestehen dürfen.
  ...
  Vorgang kann mit beliebig vielen Dimensionsleveln wiederholt werden. Die erste Zelle einer neu erstellten Tabellenkopfzeile enthält den Namen des Dimensionslevels, auf wessen Basis die Zeile beruht.
  
  Zur Erstellung der Positivliste der zu betrachtenden Dimensionslevelausprägungen klickt ein Nutzer zunächst auf das 
  ...
  \paragraph{UI-Sachen}
  \begin{itemize}
    \item im Ausdrucksbaukasten werden die Ausdrücke ausgegraut, welche keinen Sinn machen
    \item Drag und Drop verfahren
    \item Dimension und Kennzahl Symbol durch die gesamte Applikation durch
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item ReactJS
      \begin{itemize}
        \item Wiederverwendbarkeit (Geschäftsobjekt- und Kennzahlkomponente in der Berichtsansicht)
      \end{itemize}
    \item Less
    \item ES6
    \item Mocha
...
\texthl{Wie finden andere das Tool, Feedback dokumentieren, gegebenenfalls mit Fragebogen}
 ...
 spezifiziert werden, indem ihm mehrere Attribute hinzugefügt werden.
  ...
  \end{figure}
	...
	\begin{itemize}
		\item Tastaturunterstützung im Formeleditor
	\end{itemize}
% ################################################################################

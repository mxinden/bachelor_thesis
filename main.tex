...
\begin{content}
  ...
  Transaktionsdaten als auch der Stammdaten angehören. Sie bilden den
  zentralen Baustein der \textit{DataRocket}-Software. Das Geschäftsobjekt kann
  mit dem Bezugsobjekt der \textit{H2 for Reporting}-Sprache verglichen
  werden. \todo{Wo liegen die Ähnlichkeiten?}

  Einem Geschäftsobjekt können Attribute untergeordnet werden. 

  \begin{figure}[caption={Teilsprache---Geschäftsobjekt}, label={fig:img01}]
  ...
  \begin{itemize}
    ...
    \item Dimensionen
      ...
      \end{itemize}
    \item Kennzahlen
    \item Berichte \texthl{(Ist es ok, wenn ich Fakten weg lasse)}
  \end{itemize}

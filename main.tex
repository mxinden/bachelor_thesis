...
\begin{content}
  ...
  % ################################################################################
  \chapter{\textit{DataFurnace}-Softwarekomponente}
  % ################################################################################
  ...
  prototypische Softwarekomponente, \textit{DataFurnace}-Komponente, konzipiert
  und entwickelt werden, welche es ermöglicht Unternehmensdatenanalysen, mit
  besonderem Fokus auf die Datenqualitätsanalyse, in der
  \textit{DataFurnace}-Sprache zu modellieren.  Sie soll zunächst als
  alleinstehendes Softwareprodukt existieren, welches aber auch im soeben
  ...
  \paragraph{Funktionale Ziele}
  \begin{enumerate}
    \item Anwendungssoftware für die \textit{DataFurnace}-Sprache
    \item Modellierung von Dimensionen
    \item Spezifikation von Kennzahlen
    \item Kombination zu Berichten
  \end{enumerate}
  \paragraph{Nicht-funktionale Ziele}
  \begin{enumerate}
    \item Leichter durchstrukturierter Prozess
    \item Alleinstehende Software
    \item Ein- und Ausgabewerte der
      Komponente sollen mit denen des \textit{Calculation}-Bausteins übereinstimmen.
    \item Kombinierbar mit bestehenden Konventionen und Technologien von \textit{DataRocket}
  \end{enumerate}
  \paragraph{Annahmen}
  \begin{enumerate}

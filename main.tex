...
\begin{content}
  ...
  Abhängigkeit der Änderungen entsprechend korrigiert.
  ...
  \section{Umsetzung}
	...
	\begin{figure}[caption={Systemarchitektur}, label={fig:systemarchitektur}]
		\footnotesize
    \begin{tikzpicture}[]
      ...
      \node [fill=white, right=10pt] at (nodeJS.north west) {Webserver framework};

      \node (mongoDB) [box, draw, below = 10pt of nodeJS] {Database};

      \node (docker) [container, fit= (nodeJS) (mongoDB)] {};
      ...
      \node [fill=white, draw] at (line.north west) {Data protocol};
      \node [fill=white, draw] at (line.north east) {Data protocol};

      \node (subscribe) [box, above=30pt of line] {Data subscription};
      \node (businessLogicClient) [box, above= 10pt of subscribe] {Business logic};
      \node (reactJS) [box, above= 10pt of businessLogicClient] {View library};
      \node (bootstrap) [box, above=10pt of reactJS] {CSS library};
